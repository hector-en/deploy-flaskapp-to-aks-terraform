...
function verify_and_check_aks_cluster() {
  ...
  # Source output variables from cluster-outputs.sh
  source "$AZURE_DEVOPS_SCRIPTS_DIR/conf/cluster-outputs.sh" || { echo "Failed to source cluster-outputs.sh"; exit 1; }
  ...
  if [ -z "$AKS_CLUSTER_NAME" ]; then
    echo "Error: Unable to retrieve the AKS cluster name from Terraform outputs."
    exit 1
  fi

  echo "Verifying and testing Cluster: $AKS_CLUSTER_NAME in Resource Group: $NETWORK_RESOURCE_GROUP_NAME..."

  # Check if the AKS cluster exists in the specified resource group
  local aksClusterExists=$(az aks show --name "$AKS_CLUSTER_NAME" --resource-group "$NETWORK_RESOURCE_GROUP_NAME" --query "name" -o tsv 2>/dev/null)

  if [ -z "$aksClusterExists" ]; then
    echo "AKS cluster $AKS_CLUSTER_NAME under resource group $NETWORK_RESOURCE_GROUP_NAME does not exist."
    exit 1
  else
    echo "The AKS cluster $AKS_CLUSTER_NAME has been verified to exist."

    # Retrieve the kubeconfig file for the AKS cluster
    echo "Retrieving the kubeconfig file for the AKS cluster..."
    az aks get-credentials --resource-group "$NETWORK_RESOURCE_GROUP_NAME" --name "$AKS_CLUSTER_NAME" --overwrite-existing

    # Test the connection to the AKS cluster
    echo "Testing the connection to the AKS cluster..."
    if ! kubectl get nodes; then
      echo "Failed to connect to the AKS cluster $AKS_CLUSTER_NAME."
      exit 1
    else
      echo "Successfully connected to the AKS cluster $AKS_CLUSTER_NAME."
    fi
  fi
}
